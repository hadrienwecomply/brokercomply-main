...
import { requestPubAuditPdf } from "@/lib/pub-audit.server";
...
/**
 * Save the latest edits and trigger the n8n pub-report workflow
...
export async function POST(
  ...
  const res = await requestPubAuditPdf(auditId, body.edits);
  if (!res.found) {
